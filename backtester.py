...
def simulate_alternative(
    ...
    states, trader, profits_by_symbol, balance_by_symbol, traderData = trades_position_pnl_run(states, max_time, profits_by_symbol, balance_by_symbol, credit_by_symbol, unrealized_by_symbol)
...
def trades_position_pnl_run(
        ...
        for time, state in states.items():
            ...
            orders, _, traderData  = trader.run(state)
        ...
        return states, trader, profits_by_symbol, balance_by_symbol, traderData
...
if __name__ == "__main__":
    trader = Trader()
    max_time = 999000#int(input("Max timestamp (1-9)->(1-9)(00_000) or exact number): ") or 999000)
    if max_time < 10:
        max_time *= 100000
    round = 1#int(input("Input a round (blank for 4): ") or 4)
    day = -1#int(input("Input a day (blank for random): ") or random.randint(1, 3))
